...
import { Send, PenLine, Info, CheckCircle, XCircle, Clock, RotateCcw, Lock, AlertTriangle, Trash2, Eye, FileText, GraduationCap } from 'lucide-react';
...
export default function ProposeTitlePage() {
    ...
    const [proposalToCancel, setProposalToCancel] = useState<number | null>(null);
    const [formData, setFormData] = useState<ProposeTitleFormData | null>(null);
    ...
    const watchedSupervisor = watch('proposed_supervisor_id');

    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [groupRes, lecturerRes, proposalRes, bidsRes, periodRes] = await Promise.all([
            ...
            setRegisteredPeriod(periodRes.data?.period || null);
    ...
    const handlePreview = async (data: ProposeTitleFormData) => {
        setFormData(data);
    ...
    const handleCancelProposal = (proposalId: number) => {
        setProposalToCancel(proposalId);
    ...
    const confirmCancelProposal = async () => {
        ...
        try {
            const res = await api.delete(`/mahasiswa/proposal/${proposalToCancel}`);
    ...
    const getSelectedSupervisor = () => {
        if (!watchedSupervisor) return null;
        return lecturers.find(l => l.id.toString() === watchedSupervisor);
    };
    ...
    return (
        <div className="space-y-4 max-w-5xl mx-auto">
            ...
            <Breadcrumb>
                <BreadcrumbList>
                    <BreadcrumbItem>
                        <BreadcrumbLink href="/mahasiswa/dashboard">Home</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink href="/mahasiswa/titles">Titles</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage>Propose Title</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            ...
            <div className="space-y-3">
                ...
                {!hasGroup && (
                    <Alert>
                        ...
                        <AlertDescription>
                            You must <Link href="/mahasiswa/group" className="font-medium underline">create a group</Link> first before proposing a title.
                        </AlertDescription>
                    </Alert>
                )}
                ...
                {hasApprovedProposal && (
                    <Alert>
                        ...
                        <AlertDescription>
                            Your title proposal has been approved. Visit <Link href="/mahasiswa/group" className="font-medium underline">My Group</Link> to see your finalized project.
                        </AlertDescription>
                    </Alert>
                )}
                    ...
                    <form onSubmit={handleSubmit(handlePreview)}>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    <CardTitle>
                                        {editingProposal 
                                            ? (editingProposal.supervisor_approval_status === 'PENDING' ? 'Edit Proposal' : 'Resubmit Proposal')
                                            : 'New Title Proposal'
                                        }
                                    </CardTitle>
                                    <CardDescription>
                        ...
                        <CardContent className="grid gap-6 pb-6">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Title Information</h3>
                                <Controller
                                    name="title"
                                    control={control}
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Problem & Scope</h3>
                                <Controller
                            ...
                            {/* Section 3: Specializations & Supervisor */}
                            <div className="space-y-6">
                                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Specializations & Supervisor</h3>
                                <Controller
                        ...
                        <CardFooter className="flex justify-between gap-2 border-t pt-6">
                            ...
                            <Button type="submit" disabled={isSubmitting}>
                                {isSubmitting ? <div className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <Eye className="mr-2 h-4 w-4" />}
                                Preview & Submit
                            </Button>
                        </CardFooter>
                    </form>
            ...
            {proposals.length > 0 && (
                <div className="space-y-4">
                        ...
                        <h2 className="text-lg font-semibold flex items-center gap-2">
                            <FileText className="h-5 w-5" /> Proposal History
                        </h2>
                    ...
                    <div className="space-y-4">
                        {proposals.map((proposal) => (
                            <Card 
                                key={proposal.id} 
                                className={`border-l-4 ${getStatusBorderColor(proposal.supervisor_approval_status)} hover:shadow-md transition-shadow`}
                            >
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {proposal.specializations && proposal.specializations.length > 0 && (
                                        <div className="flex flex-wrap gap-1">
                                            {proposal.specializations.map(s => (
                                    ...
                                    {proposal.problem_statement && (
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground line-clamp-2">{proposal.problem_statement}</p>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-muted-foreground pt-3 border-t border-grey-100">
                                        <span>Submitted: {new Date(proposal.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                                        {proposal.updated_at !== proposal.created_at && (
                                            <span>Updated: {new Date(proposal.updated_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                                        )}
                                    </div>
                                </CardContent>
                                ...
                                {proposal.supervisor_approval_status === 'REJECTED' && !hasPendingProposal && canCreateProposal && (
                                    <CardFooter className="flex justify-end gap-2 border-t border-grey-100 pt-4">
                                ...
                                {['PENDING', 'UNDER_REVIEW'].includes(proposal.supervisor_approval_status) && canEditProposal(proposal) && (
                                    <CardFooter className="flex justify-end gap-2 border-t border-grey-100 pt-4">
                                        <Button variant="outline" size="sm" onClick={() => handleCancelProposal(proposal.id)}>
                                            <Trash2 className="mr-2 h-4 w-4" /> Batalkan
                                        </Button>
            ...
            {proposals.length === 0 && !showForm && canPropose && (
                <Card className="border-dashed border-grey-200">
                    <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                        <PenLine className="h-12 w-12 text-muted-foreground/50 mb-4" />
                        <h3 className="text-lg font-semibold text-grey-600 mb-1">No proposals yet</h3>
                        <p className="text-sm text-muted-foreground max-w-md mb-4">
                            Submit your own capstone title proposal to a supervisor.
                        </p>
                        <Button onClick={() => { setEditingProposal(null); setShowForm(true); }}>
                            <PenLine className="mr-2 h-4 w-4" /> New Proposal
                        </Button>
            ...
            {/* Footer Metadata */}
            <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-grey-100 pt-4">
                <span>
                    {proposals.length} proposal{proposals.length !== 1 ? 's' : ''} · {proposals.filter(p => p.supervisor_approval_status === 'APPROVED').length} approved
                </span>
                {registeredPeriod && (
                    ...
                    <div className="space-y-4 py-4">
                        {formData && (
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <div className="text-sm font-semibold text-muted-foreground">Problem Statement</div>
                                    <p className="text-sm text-muted-foreground">{formData.problem_statement}</p>
                                </div>
                                
                                <div className="space-y-2">
                                    <div className="text-sm font-semibold text-muted-foreground">Scope</div>
                                    <p className="text-sm text-muted-foreground">{formData.scope}</p>
                                </div>
                                
                                <div className="space-y-2">
                                    <div className="text-sm font-semibold text-muted-foreground">Specializations</div>
                                    <div className="flex flex-wrap gap-1">
                                ...
                                <div className="space-y-2">
                                    <div className="text-sm font-semibold text-muted-foreground">Supervisor</div>
                                    <div className="flex items-center gap-2">
                                        <GraduationCap className="h-4 w-4 text-muted-foreground" />
                                        <p className="text-sm">
                                            {getSelectedSupervisor()?.name || 'Not selected'} 
                                            {getSelectedSupervisor() && (
                                                <span className="text-muted-foreground"> ({getSelectedSupervisor()?.email})</span>
                                            )}
                                        </p>
                                    </div>
                                </div>
