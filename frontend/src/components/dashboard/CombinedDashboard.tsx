...
interface DashboardData {
  admin: {
    ...
    recentGroups: Array<{ id: number; label: string; subtitle: string; status: { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }; href: string }>;
  };
  dosen: {
    ...
    recentSubmissions: Array<{ id: number; label: string; subtitle: string; status: { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }; href: string }>;
  };
}
...
export default function CombinedDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [
          periodsRes, groupsRes, supervisedRes, evalCountRes,
  ...
  return (
    <SidebarProvider>
      ...
      <main className="w-full flex flex-col h-screen overflow-hidden">
        <header className="flex items-center gap-2 border-b px-4 py-3 shrink-0 bg-background z-10">
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <StatsCard
                  ...
                  icon={ClipboardCheck}
                  variant={data?.admin.pendingFinalization ? 'warning' : 'default'}
                />
                <StatsCard
                  title="TA Defenses"
                  value={0}
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <StatsCard
                  ...
                  icon={Star}
                  variant={data?.dosen.pendingEvaluations ? 'warning' : 'default'}
                />
                <StatsCard
